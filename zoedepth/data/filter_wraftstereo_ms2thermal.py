...
class MS2DataLoader:
    def __init__(self, config, mode, modality='rgbtd', **kwargs):
        ...
        b_filterw_dis = config.get("b_filterw_dis", False)  # Fitler the GT depth with the disparity from RAFTStereonet
        ...
        elif mode in ['online_eval', 'val', 'test_day', 'test_night', 'test_rainy', 'train_split0', 'train_split1']: # ERROR
            if mode == 'online_eval':
                mode = 'val'
...
class MS2Dataset(torch.utils.data.Dataset):
    def __init__(self, data_root, split_path, mode, min_depth=0.001, max_depth=80, 
                ...
                rearr_thermal=False, b_filterw_dis=False):
        """
        ...
        self.mode = mode
        self.b_filterw_dis = b_filterw_dis
        ...
        check_choices('Modality', modality, ['rgbtd', 'rgbd', 'td'])
        check_choices('Mode', mode, ['train', 'val', 'test_day', 'test_night', 'test_rainy', 'train_split0', 'train_split1'])
    ...
    def __getitem__(self, idx):
        ...
        # Read the RaftStereo disparity if needed
        raftstereo_depth_rgb = None
        raftstereo_depth_thr = None
        if self.b_filterw_dis:
            # Load the disparity for rgb
            left_disparity_path = rgb_path.replace('img_left', 'raftstereo_disp_left') # raftstereo_disp_left is with negative disparity
            left_disparity_path = left_disparity_path.replace('.png', '.npy')
            left_disparity = np.load(left_disparity_path).astype(np.float32)
            raftstereo_depth_rgb = rgb_proj_matrix[0,0]*pose_rgb_l2r[0,3] / left_disparity # (h, w), pose_rgb_l2r[0,3] is negative
            # print("############## (raftstereo_depth>0).sum(), (raftstereo_depth<0).sum(): ", (raftstereo_depth>0).sum(), (raftstereo_depth<0).sum())
            # print("############## raftstereo_depth.shape: ", raftstereo_depth.shape, raftstereo_depth.min(), raftstereo_depth.max(), raftstereo_depth.mean())

            # Load the disparity for thermal
            left_disparity_thr_path = image_path.replace('img_left', 'raftstereo_disp_left_sgm')  # raftstereo_disp_left is with negative disparity
            left_disparity_thr_path = left_disparity_thr_path.replace('.png', '.npy')
            left_disparity_thr = np.load(left_disparity_thr_path).astype(np.float32)
            raftstereo_depth_thr = -thermal_proj_matrix[0, 0] * pose_ther_l2r[0, 3] / left_disparity_thr  # (h, w), pose_rgb_l2r[0,3] is negative
            # print("--------------------------- raftstereo_depth_thr: ", raftstereo_depth_thr.shape, (raftstereo_depth_thr[raftstereo_depth_thr>0]).shape, raftstereo_depth_thr)
        ...
        if self.b_filterw_dis:
            raftstereo_depth_rgb = ToTensorV2()(image=raftstereo_depth_rgb)['image']
            raftstereo_depth_thr = ToTensorV2()(image=raftstereo_depth_thr)['image']
        ...
        if self.modality == 'rgbtd':
            sample = {
                ...
                'ther_depth_raftstereo': raftstereo_depth_thr,

                'rgb_image': rgb_img,
                'rgb_image_right': rgb_img_right,
                'rgb_depth': rgb_depth_gt,
                'pose_rgb_l2r': pose_rgb_l2r,
                'rgb_depth_raftstereo': raftstereo_depth_rgb,
...
def get_lr_consistency_mask(depth_cam1, intrinsics_cam1, intrinsics_cam2, pose_cam1_to_cam2, rgb_img_raw, rgb_img_right_raw, l1_thresh=0.9, ssim_thresh=0.9):
    ...
    # print("============== Keep depth ratio 1: ", final_mask.sum(dim=(1, 2)) / mask_photometric_error.sum(dim=(1, 2)))
    return final_mask # (B, H, W)

if __name__== "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--split', type=str, default="val", choices=['train', 'val', 'test_day', 'test_night', 'test_rainy', 'train_split0', 'train_split1'],
                        help="the split for reading the sequences in {split}.txt")
    ...
    config["b_filterw_dis"]=True

    mode = args.split
    loader = MS2DataLoader(config=config, mode=mode, modality='rgbtd')
    sample_id=0
    for data in loader.data:
        ...
        ######### Filter the depth of left RGB with left-right RGB consistency
        ...
        rgb_final_mask = get_lr_consistency_mask(rgb_depth, sample['rgb_intrinsics'].cuda(), sample['rgb_intrinsics'].cuda(), sample['pose_rgb_l2r'].cuda(), rgb_img_raw, rgb_img_right_raw,
                                                 l1_thresh=0.9, ssim_thresh=0.9) # (B, H, W)

        ######### Filter the depth of left RGB with RaftStereo depth consistency
        if config["b_filterw_dis"]:
            rgb_depth_raftstereo = sample['rgb_depth_raftstereo'].cuda() # (B, 1, H, W)
            rgb_depth_raftstereo = rgb_depth_raftstereo.permute(0, 2, 3, 1).contiguous()  # (B, H, W, 1)
            diff = (rgb_depth - rgb_depth_raftstereo).abs() # (B, H, W, 1)
            input_maks =  (rgb_depth > 0.001) & (rgb_depth_raftstereo > 0.001) & (rgb_depth < 80.0) & (rgb_depth_raftstereo < 80.0)
            # print("###################### Analyze the diff, mean, median, max, min: ", diff[input_maks].mean(), diff[input_maks].median(), diff[input_maks].max(), diff[input_maks].min())
            raftstereo_rgb_mask = input_maks & (diff < 0.12 * rgb_depth)
            # print("============== Keep depth ratio 2: ", raftstereo_mask.sum(dim=(1, 2, 3)) / input_maks.sum(dim=(1, 2, 3)))
            raftstereo_rgb_mask = raftstereo_rgb_mask.squeeze(dim=-1)
            rgb_final_mask = rgb_final_mask & raftstereo_rgb_mask
            print("============== Keep depth ratio, rgb: ", rgb_final_mask.sum(dim=(1, 2)) / input_maks.sum(dim=(1, 2)))
        ...
        ######### Filter the depth of left Thermal with left-right thermal consistency
        ...
        ther_final_mask = None
        ther_final_mask = get_lr_consistency_mask(ther_depth, sample['intrinsics'].cuda(), sample['intrinsics'].cuda(),
                                                 sample['pose_ther_l2r'].cuda(), ther_img_raw, ther_img_right_raw, l1_thresh=1.65, ssim_thresh=0.80) # l1_thresh=1.65, ssim_thresh=0.75  # l1_thresh=1.6, ssim_thresh=0.55




        ######### Filter the depth of left Thermal with RaftStereo depth consistency
        if config["b_filterw_dis"]:
            ther_depth_raftstereo = sample['ther_depth_raftstereo'].cuda() # (B, 1, H, W)
            ther_depth_raftstereo = ther_depth_raftstereo.permute(0, 2, 3, 1).contiguous()  # (B, H, W, 1)
            diff = (ther_depth - ther_depth_raftstereo).abs() # (B, H, W, 1)
            ther_input_maks =  (ther_depth > 0.001) & (ther_depth_raftstereo > 0.001) & (ther_depth < 80.0) & (ther_depth_raftstereo < 80.0)
            # print("###################### Analyze the diff, mean, median, max, min: ", diff[input_maks].mean(), diff[input_maks].median(), diff[input_maks].max(), diff[input_maks].min())
            raftstereo_thr_mask = ther_input_maks & (diff < 0.14 * ther_depth)
            # print("============== Keep depth ratio 2, thermal: ", raftstereo_thr_mask.sum(dim=(1, 2, 3)) / ther_input_maks.sum(dim=(1, 2, 3)))
            raftstereo_thr_mask = raftstereo_thr_mask.squeeze(dim=-1)
            ther_final_mask = ther_final_mask & raftstereo_thr_mask if ther_final_mask is not None else raftstereo_thr_mask
            print("============== Keep depth ratio, thermal: ", ther_final_mask.sum(dim=(1, 2)) / ther_input_maks.sum(dim=(1, 2)))
        ...
        ### Save the rgb_depth_filtered and ther_depth_filtered
        for b in range(B):
            ...
            print("Saving: ", save_rgb_depth_filename)
